/**
 * The thread store as it was before the journal (one JSON file per thread), kept verbatim as a
 * test oracle: the journal-backed store must write the same snapshot for the same calls, and an
 * app version still running this store must interoperate with journals on a synced vault.
 */
import {
...
export function createLegacyThreadStore(options: ThreadStoreOptions): ThreadStore {
