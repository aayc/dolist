...
struct DailyNotePreview: Equatable {
  ...
  static func make(_ settings: DailyNoteSettings, files: Set<String>, today: LocalDate) -> DailyNotePreview {
    var problems: [String] = []
    if settings.folder.count > SettingsRanges.folderLength { problems.append("The folder is too long.") }
    if settings.format.count > SettingsRanges.formatLength { problems.append("The format is too long.") }
    if settings.template.count > SettingsRanges.templateLength { problems.append("The template path is too long.") }
...
struct DailyNotesSettingsPane: View {
  ...
  var body: some View {
    ...
    let preview = DailyNotePreview.make(draft, files: files, today: LocalDate.today(now: model.environment.now()))
    Form {
      Section("Daily notes") {
        ...
        CommitTextField(title: "Date format", value: daily.format, prompt: "YYYY-MM-DD", monospaced: true) { format in
          draft.format = format
          update(SettingsPatch(dailyNotes: .init(format: format)))
        }
        CommitTextField(title: "Template", value: daily.template, prompt: "No template") { template in
        ...
        if preview.templateMissing {
          SettingsNote(text: "The template note isn't in the vault; new daily notes start empty.", tone: Theme.warning)
        }
        SettingsNote(text: "Moment.js tokens, e.g. YYYY-MM-DD or YYYY/MM/YYYY-MM-DD (nested folders). Same settings as Obsidian's daily notes.")
      }
      Section("Weekly notes (⇧⌘W)") {
        ...
        CommitTextField(title: "Format", value: weekly.format, prompt: "gggg-[W]ww", monospaced: true) {
        ...
        LabeledContent("This week's note") {
          Text(DailyNotes.weeklyPath(for: LocalDate.today(now: model.environment.now()), settings: weekly))
