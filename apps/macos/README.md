...
| `Packages/DailyDoListUI` | What the shell, the agent UI and the editor share: the palette (`Theme`), the main-actor scheduler (`AppScheduler`, with `ManualScheduler` for tests), the app's one tooltip (`TooltipCenter`, `.tooltip(…)`), keycaps (`KeyShortcut`, `Keycaps`), `.pointingHandCursor()`, `IconButton`, and the chrome and accent button styles. `DailyDoListUITestSupport` finds tooltips in tests and draws them into snapshots. |
