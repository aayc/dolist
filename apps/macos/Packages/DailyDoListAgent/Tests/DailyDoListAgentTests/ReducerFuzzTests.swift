...
struct ReducerFuzzTests {
  ...
  struct Generator {
    ...
    mutating func messageId(_ thread: String) -> String {
      "msg_\(thread)_\(Int.random(in: 0...3, using: &rng))"
    }
    ...
    mutating func record(note: String? = nil) -> TaskAgentRecord {
      TaskAgentRecord(
        ...
        date: "2026-09-23", text: pick(["Buy milk", "Book a table"]),
        line: Int.random(in: 0...20, using: &rng),
        status: pick(ReducerFuzzTests.statuses),
        threadId: bool() ? pick(ReducerFuzzTests.threadIds) : nil,
    ...
    mutating func summary(_ id: String? = nil) -> ThreadSummary {
      ThreadSummary(
        id: id ?? pick(ReducerFuzzTests.threadIds),
        taskId: bool() ? pick(ReducerFuzzTests.tasks) : nil,
        notePath: bool() ? pick(ReducerFuzzTests.notes) : nil,
        title: pick(["Buy milk", "Buy oat milk"]),
    ...
    mutating func message(_ thread: String) -> ThreadMessage {
      ...
      case 4:
        return .status(
          StatusMessage(
            id: id, author: "system", createdAt: 1, status: pick(ReducerFuzzTests.statuses)))
      default:
        return .approval(
          ApprovalMessage(
            id: id, author: "system", createdAt: 1, approvalId: pick(ReducerFuzzTests.approvalIds)))
      }
    }

    mutating func approval() -> ApprovalRequest {
      ApprovalRequest(
        id: pick(ReducerFuzzTests.approvalIds),
        threadId: bool() ? pick(ReducerFuzzTests.threadIds + ["thr_zz"]) : nil,
        taskId: nil, toolName: "browser_click", input: ["element": "Place order"],
        summary: "Place order",
    ...
    mutating func loadedThread(_ id: String? = nil) -> AgentThread {
      ...
      return AgentThread(
        id: id, taskId: bool() ? pick(ReducerFuzzTests.tasks) : nil,
        notePath: pick(ReducerFuzzTests.notes),
        title: pick(["Buy milk", "Buy oat milk"]), status: pick(ReducerFuzzTests.statuses),
        createdAt: 1,
    ...
    mutating func event() -> ServerEvent {
      ...
      case 12..<17:
        ...
        return .threadDelta(
          ThreadDeltaEvent(
            threadId: thread, messageId: bool() ? messageId(thread) : "msg_unknown",
            delta: pick([" more", "…", "😀"])))
      case 17, 18: return .approvalUpsert(approval())
      default:
        return .agentStatus(
          Fixture.status(enabled: bool(), running: Int.random(in: 0...3, using: &rng)))
      }
    }
  ...
  static func expectConsistent(
    _ state: AgentState, sourceLocation: SourceLocation = #_sourceLocation
  ) {
    var notesOfTask: [String: [String]] = [:]
    for (note, records) in state.recordsByNote {
      #expect(
        Set(records.map(\.taskId)).count == records.count, "tasks are unique within \(note)",
        sourceLocation: sourceLocation)
      for record in records {
        #expect(
          record.notePath == note, "record stored under its note", sourceLocation: sourceLocation)
        notesOfTask[record.taskId, default: []].append(note)
      }
    }
    for (task, notes) in notesOfTask {
      #expect(
        notes.count == 1, "task \(task) lives in one note (\(notes))",
        sourceLocation: sourceLocation)
    }
    ...
    for (id, thread) in state.loadedThreads {
      ...
      #expect(
        Set(ids).count == ids.count, "messages of \(id) are unique", sourceLocation: sourceLocation)
    }
    for (id, approval) in state.approvals {
      #expect(approval.id == id, sourceLocation: sourceLocation)
    }
    for key in state.deltaPlaceholders {
      let message = state.loadedThreads[key.threadId]?.messages.first { $0.id == key.messageId }
      #expect(
        message?.kind == "text", "placeholders point at text messages",
        sourceLocation: sourceLocation)
    }
  }
    ...
    for _ in 0..<80 {
      ...
      case .taskRecord(let record):
        ...
        if let previous, let current {
          #expect(current.updatedAt >= previous.updatedAt, "a stale record never wins")
        }
      case .threadMessage(let e):
        if before.loadedThreads[e.threadId] == nil {
          ...
          #expect(
            state.loadedThreads[e.threadId]?.messages.contains { $0.id == e.message.id } == true)
        }
      ...
      case .threadUpsert(let summary):
        if state.threads[summary.id] == summary, let thread = state.loadedThreads[summary.id] {
          #expect(
            thread.status == summary.status && thread.title == summary.title,
            "loaded thread follows its summary")
        }
      default:
        break
      }
  ...
  func deliveringAnEventTwiceIsTheSameAsOnce(seed: Int) {
    ...
    for _ in 0..<Int.random(in: 0...40, using: &generator.rng) {
      _ = generator.step(&state, history: &history)
    }
    for _ in 0..<10 {
  ...
  func aResyncConvergesToTheDaemonsSnapshot(seed: Int) {
    ...
    for thread in loaded {
      _ = state.applyThreadResponse(ThreadResponse(thread: thread, approvals: []))
    }
