...
enum SampleImages {
  ...
  static let browserFrame: String = render(width: browserSize.width, height: browserSize.height) {
    size in
    ...
    for (index, item) in ["Menu", "Reservations", "Private dining", "Contact"].enumerated() {
      text(
        item, at: NSPoint(x: 780 + index * 120, y: 29), size: 16, weight: .medium, color: 0xE8DCC6)
    }
    // Photo placeholder
    ...
    text(
      "Patio · Summer menu", at: NSPoint(x: 788, y: 380), size: 18, weight: .semibold,
      color: 0xFFFFFF)
    // Form
    text("Reserve a table", at: NSPoint(x: 48, y: 120), size: 40, weight: .bold, color: 0x1F1F1F)
    text(
      "Friday, 7:00 PM · Party of 4 · Patio", at: NSPoint(x: 50, y: 178), size: 18,
      weight: .regular, color: 0x5C5C5C)
    let fields = [
    ...
    text(
      "Complete reservation", at: NSPoint(x: button.minX + 62, y: button.minY + 17), size: 20,
      weight: .semibold, color: 0xFFFFFF)
    text(
      "You'll get a confirmation by text message.", at: NSPoint(x: 776, y: 700), size: 14,
      weight: .regular, color: 0x7A7A7A)
  }

  static let desktopFrame: String = render(width: desktopSize.width, height: desktopSize.height) {
    size in
    ...
    text(
      "Browser   File   Edit   View   History", at: NSPoint(x: 20, y: 6), size: 14, weight: .medium,
      color: 0x222222)
    // Window
    let window = NSRect(x: 180, y: 110, width: 1080, height: 680)
    fill(window, 0xFFFFFF, radius: 12)
    fill(
      NSRect(x: window.minX, y: window.minY, width: window.width, height: 44), 0xF1F1F4, radius: 12)
    for (index, color) in [0xFF5F57, 0xFEBC2E, 0x28C840].enumerated() {
      fill(
        NSRect(
          x: window.minX + 18 + CGFloat(index) * 22, y: window.minY + 15, width: 13, height: 13),
        UInt32(color), radius: 6.5)
    }
    text(
      "example-roasters.example/reorder", at: NSPoint(x: 560, y: 124), size: 14, weight: .regular,
      color: 0x6A6A6A)
    text("Example Roasters", at: NSPoint(x: 230, y: 190), size: 30, weight: .bold, color: 0x2A1E17)
    text(
      "Your usual order", at: NSPoint(x: 232, y: 236), size: 17, weight: .regular, color: 0x6A6A6A)
    for index in 0..<2 {
      ...
      fill(
        NSRect(x: card.minX + 24, y: card.minY + 24, width: 140, height: 180),
        index == 0 ? 0x6B4A34 : 0x8C5A3C, radius: 8)
      text(
        index == 0 ? "House Blend" : "Ethiopia Natural",
        at: NSPoint(x: card.minX + 190, y: card.minY + 36), size: 22, weight: .semibold,
        color: 0x2A1E17)
      text(
        index == 0 ? "Whole bean · 1 lb · $16" : "Whole bean · 12 oz · $19",
        at: NSPoint(x: card.minX + 190, y: card.minY + 72), size: 15, weight: .regular,
        color: 0x6A6A6A)
    }
    let button = NSRect(
      x: addToCartCenter.x - 110, y: addToCartCenter.y - 24, width: 220, height: 48)
    fill(button, 0x2A1E17, radius: 10)
    text(
      "Add to cart", at: NSPoint(x: button.minX + 56, y: button.minY + 13), size: 18,
      weight: .semibold, color: 0xFFFFFF)
  }
  ...
  private static func text(
    ...
    let attributes: [NSAttributedString.Key: Any] = [
      .font: font, .foregroundColor: NSColor(rgb: color),
    ]
    NSGraphicsContext.saveGraphicsState()
