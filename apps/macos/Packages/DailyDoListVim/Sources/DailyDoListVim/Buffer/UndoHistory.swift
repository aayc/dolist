...
struct UndoHistory {
  ...
  mutating func record(changes: ChangeSet, inverted: ChangeSet?, startSelection: EditorSelection, selectionSet: Bool, time: Double, userEvent: String?) {
    if !changes.isEmpty, let inverted {
      addChanges(HistEvent(changes: inverted, startSelection: startSelection, selectionsAfter: []), time: time, userEvent: userEvent)
  ...
  private mutating func addChanges(_ event: HistEvent, time: Double, userEvent: String?) {
    if let lastEvent = done.last, let lastChanges = lastEvent.changes, !lastChanges.isEmpty, let changes = event.changes,
      Self.isJoinable(userEvent),
      (lastEvent.selectionsAfter.isEmpty && time - prevTime < newGroupDelay && Self.isAdjacent(lastChanges, changes))
        || userEvent == "input.type.compose"
    {
      let joined = HistEvent(changes: changes.compose(lastChanges), startSelection: lastEvent.startSelection, selectionsAfter: [])
  ...
  private mutating func addSelection(_ selection: EditorSelection, time: Double, userEvent: String?) {
    ...
    if !last.isEmpty && time - prevTime < newGroupDelay && userEvent == prevUserEvent, let userEvent,
      userEvent == "select" || userEvent.hasPrefix("select."), Self.eqSelectionShape(last[last.count - 1], selection)
    {
      return
    }
  ...
  private static func updateBranch(_ branch: [HistEvent], _ to: Int, _ maxLen: Int, _ newEvent: HistEvent) -> [HistEvent] {
  ...
  private static func addSelection(_ branch: [HistEvent], _ selection: EditorSelection) -> [HistEvent] {
  ...
  func pop(undo: Bool, currentSelection: EditorSelection) -> (changes: ChangeSet, selection: EditorSelection?, rest: [HistEvent], remembered: EditorSelection)? {
    let branch = undo ? done : undone
    guard let event = branch.last, let changes = event.changes else { return nil }
    let remembered = event.selectionsAfter.first
  ...
  mutating func recordPop(undo: Bool, rest: [HistEvent], applied: ChangeSet, inverted: ChangeSet, remembered: EditorSelection, startSelection: EditorSelection) {
    var other = undo ? undone : done
    if !applied.isEmpty {
      other = Self.updateBranch(other, other.count, minDepth, HistEvent(changes: inverted, startSelection: remembered, selectionsAfter: []))
    } else {
