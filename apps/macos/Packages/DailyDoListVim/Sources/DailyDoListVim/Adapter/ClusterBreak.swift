...
enum ClusterBreak {
  ...
  private static let ranges: [(UInt32, UInt32)] = {
    ...
    let values = numbers.split(separator: ",", omittingEmptySubsequences: false).map { $0.isEmpty ? UInt32(1) : UInt32($0, radix: 36)! }
  ...
  static func isExtendingChar(_ code: UInt32) -> Bool {
    ...
    while lo < hi {
      let mid = (lo + hi) >> 1
      if code < ranges[mid].0 { hi = mid } else if code >= ranges[mid].1 { lo = mid + 1 } else { return true }
    }
    return false
  }

  private static func isRegionalIndicator(_ code: UInt32) -> Bool { code >= 0x1F1E6 && code <= 0x1F1FF }
  ...
  static func find(_ units: [UInt16], _ pos: Int, forward: Bool = true, includeExtending: Bool = true) -> Int {
