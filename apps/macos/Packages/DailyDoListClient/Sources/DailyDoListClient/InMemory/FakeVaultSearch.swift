...
enum FakeVaultSearch {
  ...
  static func search(_ query: String, in notes: [Note], limit: Int) -> [SearchHit] {
    ...
    var terms: [String] = []
    for term in needle.split(whereSeparator: \.isWhitespace).map(String.init)
    ...
    for note in sorted {
      let name = (matchPath ? String(note.path.dropLast(3)) : FakeVaultPaths.stem(note.path))
        .lowercased()
    ...
    for note in sorted where note.size <= maxFileBytes {
      for (index, line) in note.content.textLines.enumerated() {
        ...
        hits.append(
          SearchHit(
            path: note.path, kind: .content, line: index,
            preview: preview(text, start, first.utf16.count)))
        if hits.count >= limit { return hits }
      }
    }
    return hits
  ...
  static func preview(_ line: String, _ matchStart: Int, _ matchLength: Int) -> String {
    ...
    if units.count <= previewCharacters {
    ...
    let body = String(decoding: units[start..<end], as: UTF16.self).trimmingCharacters(
