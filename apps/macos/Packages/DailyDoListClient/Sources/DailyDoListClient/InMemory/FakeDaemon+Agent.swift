...
enum FakeTaskParser {
  ...
  private static func parse(_ line: Substring, line index: Int) -> ParsedTask? {
    ...
    if let marker = rest.first, "-*+".contains(marker) {
      ...
      guard (1...9).contains(digits.count), let end = rest.dropFirst(digits.count).first,
        end == "." || end == ")"
      else {
        return nil
      }
...
extension FakeDaemon {
  ...
  static func agentUnavailable() -> DaemonClientError {
    .http(
      status: 503,
      body: ApiErrorBody(error: .agentUnavailable, message: "The agent runtime is not running"))
  }
  ...
  func status() -> AgentStatusResponse {
    ...
    return AgentStatusResponse(
      mode: enabled ? .mock : .off, enabled: settings.agent.enabled,
      model: settings.agent.agentModel,
  ...
  func recordsFor(_ notePath: String) -> [TaskAgentRecord] {
    records.values.filter { $0.notePath == notePath }.sorted {
      ($0.line, $0.taskId) < ($1.line, $1.taskId)
    }
  }
  ...
  func thread(_ id: String) throws(DaemonClientError) -> ThreadResponse {
    ...
    let approvals = approvals.values.filter { $0.threadId == id }.sorted {
  ...
  func postMessage(threadId: String, text: String) throws(DaemonClientError) -> ThreadActionResponse
  {
    ...
    jobs[id] = Job(
      ...
      beats: say("orchestrator", reply, after: 500) + [Job.Beat(delay: 0, step: .end)],
  ...
  func cancelThread(_ threadId: String) throws(DaemonClientError) -> ThreadActionResponse {
    ...
    if let taskId = thread.taskId,
      let index = waitingJobs.firstIndex(where: { $0.taskId == taskId })
    {
  ...
  func decideApproval(_ id: String, _ decision: ApprovalDecisionRequest) throws(DaemonClientError)
    -> ApprovalRequest
  {
    let id = try RequestGuards.runtimeID(id, "id")
    ...
    guard approval.isPending else {
      throw .approvalConflict(
        ApprovalConflictResponse(
          message: "Approval is already \(approval.status)", approval: approval))
    }
    ...
    emitStatus()
    if var job = jobs.values.first(where: { $0.waitingApproval == id }),
      let risky = job.script?.risky
    {
      job.waitingApproval = nil
      let author = job.script?.author ?? "orchestrator"
      job.beats =
        Array(job.beats[..<job.index])
  ...
  func observeNote(_ path: String, content: String, initial: Bool = false) {
    ...
    for (index, task) in parsed.enumerated() {
      if let old = assigned[index] {
        ...
        let added = TrackedTask(
          id: nextID("tsk"), text: task.text, line: task.line, isOpen: task.isOpen)
        next.append(added)
    ...
    var changed = false
    for task in next {
      guard var record = records[task.id], record.line != task.line || record.text != task.text
      else { continue }
  ...
  func markRead(_ threadId: String) {
    guard let taskId = threads[threadId]?.taskId, let record = records[taskId], record.unread > 0
    else { return }
  ...
  func settled(_ taskId: String, token: Int) {
    guard settleTokens[taskId] == token, simulation == .enabled, settings.agent.enabled,
      records[taskId] == nil,
  ...
  func startJob(_ path: String, taskId: String, text: String, line: Int, threadId: String?) {
    ...
    if records[taskId] == nil {
      ...
      records[taskId] = TaskAgentRecord(
        taskId: taskId, notePath: path, date: date, text: text, line: line, status: .idle,
        threadId: threadId,
    ...
    var record = TaskAgentRecord(
      taskId: taskId, notePath: path, date: date, text: text, line: line, status: .triaging,
    ...
    jobGeneration += 1
    jobs[taskId] = Job(
      id: taskId, taskId: taskId, threadId: record.threadId, script: script,
      beats: taskBeats(script),
      generation: jobGeneration)
    emitStatus()
  ...
  private func taskBeats(_ script: AgentScript) -> [Job.Beat] {
    ...
    for step in script.steps {
      ...
      if let page = step.page {
        ...
        beats.append(
          Job.Beat(
            delay: (step.durationMs / 2).rounded(.up),
            step: .toolFinish(.ok, preview: step.resultPreview)))
      } else {
        beats.append(
  ...
  private func decisionBeats(
    ...
    if approved {
      ...
      beats.append(
        Job.Beat(
          delay: 0,
          step: .toolFinish(.blocked, preview: note.map { "Denied: \($0)" } ?? "Denied by you")))
  ...
  func runBeat(_ jobId: String, generation: Int) {
    guard var job = jobs[jobId], job.generation == generation, job.index < job.beats.count,
      job.waitingApproval == nil
    else { return }
  ...
  private func perform(_ step: Job.Step, in jobId: String) -> Bool {
    ...
    case .begin:
      ...
      let threadId =
        job.threadId.flatMap { threads[$0] == nil ? nil : $0 }
    ...
    case .textStart(let author):
      ...
      let text = Job.StreamingText(
        id: nextID("msg"), author: author, createdAt: nowMillis, streamed: "")
      job.text = text
      jobs[jobId] = job
      pushMessage(
        threadId,
        .text(
          TextMessage(
            id: text.id, author: author, createdAt: nowMillis, role: .agent, text: "",
            streaming: true)))
    ...
    case .toolStart(let tool):
      ...
      let message = ToolCallMessage(
        id: nextID("msg"), author: job.script?.author ?? "orchestrator", createdAt: nowMillis,
    ...
    case .requestApproval(let risky):
      ...
      let approval = ApprovalRequest(
        id: nextID("apr"), threadId: threadId, taskId: taskId, toolName: risky.toolName,
        toolLabel: risky.toolLabel,
        input: risky.input, summary: risky.summary, risk: risky.risk, categories: risky.categories,
        reason: risky.reason,
        status: .pending, createdAt: nowMillis,
        expiresAt: nowMillis + Double(settings.agent.approvalTimeoutMs))
      approvals[approval.id] = approval
      job.waitingApproval = approval.id
      jobs[jobId] = job
      emit(.approvalUpsert(approval))
      let marker = ApprovalMessage(
        id: nextID("msg"), author: "system", createdAt: nowMillis, approvalId: approval.id)
    ...
    case .resume(let approved):
      ...
      setThreadStatus(
        threadId, .working, approved ? "Approved — continuing" : "Denied — skipping that step")
  ...
  func abort(_ job: Job) {
    ...
    if let text = job.text {
      replaceMessage(
        threadId, Self.agentText(text.author, text.streamed, at: text.createdAt, id: text.id))
    }
  }
  ...
  private func cancelApproval(of job: Job) {
    guard let id = job.waitingApproval, var approval = approvals[id], approval.isPending else {
      return
    }
  ...
  private func drainQueue() {
    while !waitingJobs.isEmpty, runningTaskJobs < max(1, settings.agent.maxConcurrentSubagents) {
      ...
      startJob(
        next.notePath, taskId: next.taskId, text: record.text, line: record.line,
  ...
  func createThread(taskId: String?, notePath: String?, title: String) -> String {
    ...
    threads[id] = AgentThread(
      id: id, taskId: taskId, notePath: notePath, title: title, status: .triaging,
      createdAt: nowMillis,
      updatedAt: nowMillis)
    emitThread(id)
    return id
  ...
  private func updateMessage(
    _ threadId: String, id: String, _ update: (inout ThreadMessage) -> Void
  ) {
    guard var thread = threads[threadId],
  ...
  static func agentText(_ author: MessageAuthor, _ text: String, at time: EpochMillis, id: String)
    -> ThreadMessage
  {
    .text(
      TextMessage(
        id: id, author: author, createdAt: time, role: .agent, text: text, streaming: false))
  }

  static func statusMessage(
    _ status: TaskAgentStatus, _ text: String?, at time: EpochMillis, id: String
  ) -> ThreadMessage {
    .status(StatusMessage(id: id, author: "system", createdAt: time, status: status, text: text))
  }

  private func addArtifact(
    _ threadId: String, _ artifact: AgentScript.Artifact, author: MessageAuthor
  ) {
    let id = nextID("art")
    let data = Data(artifact.content.utf8)
    let meta = ArtifactMeta(
      id: id, threadId: threadId, title: artifact.title, kind: .markdown, mimeType: "text/markdown",
      path: "\(FakeVaultPaths.sidecar)/artifacts/\(threadId)/\(id).md", size: data.count,
      createdAt: nowMillis)
    ...
    pushMessage(
      threadId,
      .artifact(
        ArtifactMessage(id: nextID("msg"), author: author, createdAt: nowMillis, artifactId: id)))
  }
  ...
  func emitFrame(_ key: SurfaceKey, _ page: AgentScript.BrowserPage) {
    ...
    emit(
      .surfaceFrame(
        SurfaceFrame(
          threadId: key.threadId, surface: key.surface, mimeType: "image/png",
          data: AgentScript.framePNGBase64,
          width: AgentScript.frameSize.width, height: AgentScript.frameSize.height, url: page.url,
          title: page.title,
