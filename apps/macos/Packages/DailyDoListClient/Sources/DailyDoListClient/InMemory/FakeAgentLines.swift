...
extension FakeDaemon {
  ...
  func writeAgentLine(_ text: String, underTask taskId: String, threadId: String) {
    ...
    while at < lines.count,
      lines[at].prefix(while: { $0 == " " || $0 == "\t" }).count > indent.count,
  ...
  func followAnchors(_ path: String, content: String) -> Bool {
    let lines = content.textLines.map {
      FakeAgentText.strip(String($0)).trimmingCharacters(in: .whitespaces)
    }
    var changed = false
