...
struct InMemoryAgentTests {
  ...
  static func client(_ clock: SimulationClock = .immediate(), seed: InMemoryDaemonClient.Seed = .empty) -> InMemoryDaemonClient {
  ...
  static func collect(_ client: InMemoryDaemonClient, _ body: () async throws -> Void) async throws -> [DaemonStreamItem] {
  ...
  @Test func researchTaskRunsToDone() async throws {
    ...
    let items = try await Self.collect(client) {
      _ = try await client.writeNote(Self.today, content: "- [ ] Research the best indoor herb garden kits\n", baseVersion: .createOnly)
    }
    ...
    #expect(Self.transitions(records.compactMap(\.summary)) == ["Reading the task…", "Researching…", "3 options"])
    let final = try #require(records.last)
    #expect(final.notePath == Self.today && final.date == "2026-09-23" && final.line == 0 && final.unread == 3)
    #expect(final.text == "Research the best indoor herb garden kits")
    #expect(records.first?.updatedAt == Self.start + 1200, "starts once the 1.2 s settle delay passed")
    ...
    #expect(events.contains { if case .taskRecords(let e) = $0 { e.records.map(\.taskId) == [final.taskId] } else { false } })

    let threadId = try #require(final.threadId)
    let response = try await client.thread(threadId)
    #expect(response.thread.status == .done && response.thread.title == final.text && response.approvals.isEmpty)
    #expect(response.thread.messages.map(\.kind) == [
    ...
    for case .text(let message) in response.thread.messages {
      let deltas = events.compactMap { event -> String? in
        if case .threadDelta(let delta) = event, delta.messageId == message.id { delta.delta } else { nil }
      }
    ...
    #expect(String(decoding: artifact.data, as: UTF8.self).hasPrefix("# Research the best indoor herb garden kits"))
  ...
  @Test func approvedRiskyTaskFinishes() async throws {
    ...
    let items = try await Self.collect(client) {
      _ = try await client.writeNote(Self.today, content: "- [ ] Order a new kettle\n", baseVersion: .createOnly)
      ...
      let decided = try await client.decideApproval(try #require(approval).id, ApprovalDecisionRequest(decision: .approve))
      #expect(decided.status == .approved && decided.scope == .once && decided.decidedAt != nil)
    }
    let pending = try #require(approval)
    #expect(pending.toolName == "browser_click" && pending.risk == .high && pending.categories == [.payment, .formSubmission])
    #expect(pending.summary.contains("Order a new kettle") && pending.expiresAt == pending.createdAt + 43_200_000)

    let events = Self.events(items)
    #expect(Self.transitions(Self.records(events).map(\.status)) == [.idle, .triaging, .working, .waitingApproval, .working, .done])
    #expect(Self.records(events).last?.summary == "Ordered · arrives in 2 days")
    let upserts = events.compactMap { if case .approvalUpsert(let a) = $0 { a.status } else { nil } }
    #expect(upserts == [.pending, .approved])

    let thread = try await client.thread(try #require(pending.threadId))
    let calls = thread.thread.messages.compactMap { if case .toolCall(let c) = $0 { c } else { nil } }
    #expect(calls.last?.toolName == "browser_click" && calls.last?.status == .ok && calls.last?.resultPreview == "Done")
    #expect(thread.thread.messages.contains { if case .approval(let m) = $0 { m.approvalId == pending.id } else { false } })
    ...
    do {
      ...
      #expect(conflict.approval.status == .approved && conflict.message == "Approval is already approved")
    }
  }
  ...
  @Test func deniedRiskyTaskIsBlockedWithTheNote() async throws {
    let client = Self.client()
    let items = try await Self.collect(client) {
      _ = try await client.writeNote(Self.today, content: "- [ ] Book a haircut for Saturday\n", baseVersion: .createOnly)
      ...
      _ = try await client.decideApproval(approval.id, ApprovalDecisionRequest(decision: .deny, note: "I'll call instead"))
    }
    let events = Self.events(items)
    let records = Self.records(events)
    #expect(Self.transitions(records.map(\.status)) == [.idle, .triaging, .working, .waitingApproval, .working, .done])
    ...
    let blocked = thread.thread.messages.compactMap { if case .toolCall(let c) = $0, c.status == .blocked { c } else { nil } }
    #expect(blocked.count == 1 && blocked.first?.resultPreview == "Denied: I'll call instead")
    let texts = thread.thread.messages.compactMap { if case .text(let t) = $0 { t.text } else { nil } }
    #expect(texts.last?.hasSuffix("> Your note: I'll call instead") == true)
    #expect(thread.approvals.first?.status == .denied && thread.approvals.first?.decisionNote == "I'll call instead")
    let statuses = thread.thread.messages.compactMap { if case .status(let s) = $0 { s.text } else { nil } }
  ...
  @Test func runsAreDeterministic() async throws {
    func scenario() async throws -> [DaemonStreamItem] {
      ...
      return try await Self.collect(client) {
        _ = try await client.writeNote(Self.today, content: "- [ ] Send the quarterly update\n- [ ] Research quiet dishwashers\n", baseVersion: .unconditional)
        let approval = try #require(try await client.approvals(status: .pending).first)
        await client.send(.threadRead(threadId: try #require(approval.threadId)))
        _ = try await client.decideApproval(approval.id, ApprovalDecisionRequest(decision: .approve, scope: .task))
      }
    }
  ...
  @Test func manualClockAdvancesStepByStep() async throws {
    ...
    let recorder = StreamRecorder(client.events())
    await client.connect()
    _ = try await client.writeNote(Self.today, content: "- [ ] Research standing desks\n", baseVersion: .createOnly)
    ...
    guard case .text(let message) = partial else { throw TimeoutError(description: "expected a streaming text message") }
  ...
  @Test func browseTasksStreamFramesToSubscribers() async throws {
    ...
    let recorder = StreamRecorder(client.events())
    await client.connect()
    _ = try await client.writeNote(Self.today, content: "- [ ] Browse for a standing desk under $300\n", baseVersion: .createOnly)
    ...
    #expect(frame.threadId == threadId && frame.surface == .browser && frame.mimeType == "image/png")
    #expect(frame.url?.hasPrefix("https://guide.example/search?q=") == true && frame.action?.kind == "navigate")
    ...
    let items = try await Self.collect(quiet) {
      _ = try await quiet.writeNote(Self.today, content: "- [ ] Browse for a standing desk\n", baseVersion: .createOnly)
    }
  ...
  @Test func cancelStopsTheJobAndRetryRestartsIt() async throws {
    let client = Self.client()
    let items = try await Self.collect(client) {
      _ = try await client.writeNote(Self.today, content: "- [ ] Pay the electricity bill\n", baseVersion: .createOnly)
      let approval = try #require(try await client.approvals(status: .pending).first)
      ...
      let calls = try await client.thread(threadId).thread.messages.compactMap { if case .toolCall(let c) = $0 { c } else { nil } }
      ...
      #expect(try await client.threads(notePath: Self.today, taskId: nil).count == 1, "the same thread is reused")
    }
    let statuses = Self.transitions(Self.records(Self.events(items)).map(\.status))
    #expect(statuses == [.idle, .triaging, .working, .waitingApproval, .cancelled, .triaging, .working, .waitingApproval])
  }

  @Test func userMessagesGetAReply() async throws {
    let client = Self.client()
    _ = try await client.writeNote(Self.today, content: "- [ ] Email the landlord about the heater\n", baseVersion: .createOnly)
    let approval = try #require(try await client.approvals(status: .pending).first)
    let threadId = try #require(approval.threadId)
    _ = try await client.postMessage(threadId: threadId, text: "  Mention it's urgent  ")
    var texts = try await client.thread(threadId).thread.messages.compactMap { if case .text(let t) = $0 { t } else { nil } }
    #expect(texts.dropLast().last?.text == "Mention it's urgent" && texts.dropLast().last?.author == "you")
    #expect(texts.last?.text == "Got it — I'll factor that in as I go." && texts.last?.author == "orchestrator")
    ...
    texts = try await client.thread(threadId).thread.messages.compactMap { if case .text(let t) = $0 { t } else { nil } }
    #expect(texts.last?.text.hasPrefix("Thanks! I've added that") == true)

    await #expect(throws: DaemonClientError.self) { try await client.postMessage(threadId: threadId, text: "   ") }
    await #expect(throws: DaemonClientError.http(status: 404, body: ApiErrorBody(error: .notFound, message: "Thread not found"))) {
  ...
  @Test func readingAThreadClearsUnread() async throws {
    let client = Self.client()
    _ = try await client.writeNote(Self.today, content: "- [ ] Research compost bins\n", baseVersion: .createOnly)
    let record = try #require(try await client.taskRecords(notePath: Self.today).first)
  ...
  @Test func onlyNewOpenTasksInWatchedDailyNotesStartWork() async throws {
    let client = Self.client()
    _ = try await client.writeNote("Projects/Plan.md", content: "- [ ] Research tents\n", baseVersion: .createOnly)
    _ = try await client.writeNote("Daily/2026-09-22.md", content: "- [ ] Research tents\n", baseVersion: .createOnly)
    _ = try await client.writeNote(Self.today, content: "- [x] Research tents\n- [ ] \n- [ ] x\n", baseVersion: .createOnly)
    #expect(try await client.threads(notePath: nil, taskId: nil).isEmpty)
    _ = try await client.writeNote("Daily/2026-09-30.md", content: "- [ ] Research tents\n", baseVersion: .createOnly)
    #expect(try await client.taskRecords(notePath: "Daily/2026-09-30.md").first?.status == .done, "the watch window reaches 7 days ahead")
  ...
  @Test func editsRestartTheSettleDelayAndTypingDefersWork() async throws {
    ...
    _ = try await client.writeNote(Self.today, content: "- [ ] Research bikes", baseVersion: .unconditional)
    await client.advance(by: .milliseconds(1000))
    #expect(try await client.taskRecords(notePath: Self.today).isEmpty, "the edit restarted the delay")
    await client.send(.editorActivity(notePath: Self.today, line: 0))
    await client.advance(by: .milliseconds(200))
    #expect(try await client.taskRecords(notePath: Self.today).isEmpty, "the user is still typing on that line")
    ...
    _ = try await client.writeNote(Self.today, content: "- [ ] Research e-bikes", baseVersion: .unconditional)
  ...
  @Test func concurrencyLimitQueuesTasks() async throws {
    ...
    _ = try await client.writeNote(Self.today, content: "- [ ] Research kettles\n- [ ] Research toasters\n", baseVersion: .createOnly)
    await client.advance(by: .milliseconds(1200))
  ...
  @Test func disabledSimulationIsAgentModeOff() async throws {
    ...
    _ = try await client.writeNote(Self.today, content: "- [ ] Order a kettle\n", baseVersion: .unconditional)
    #expect(try await client.taskRecords(notePath: Self.today).isEmpty)
    #expect(try await client.threads(notePath: nil, taskId: nil).isEmpty)
    let status = try await client.agentStatus()
  ...
  @Test func realTimeClockDrivesTheSimulation() async throws {
    ...
    _ = try await client.writeNote(path, content: "- [ ] Research rain jackets\n", baseVersion: .unconditional)
