...
struct InMemoryVaultTests {
  static func client(_ seed: InMemoryDaemonClient.Seed = .empty, agent: AgentSimulation = .enabled)
  ...
  @Test func writeReadAndVersions() async throws {
    ...
    let created = try await client.writeNote(
      "Notes/Café.md", content: "hello", baseVersion: .createOnly)
    #expect(
      created
        == WriteNoteResponse(
          path: "Notes/Café.md", version: "106f3a63cd7226", mtime: 1_790_155_800_000))
    let note = try await client.readNote("Notes//./Café.md")
    #expect(
      note
        == NoteResponse(
          path: "Notes/Café.md", content: "hello", version: "106f3a63cd7226",
          mtime: 1_790_155_800_000))
    let updated = try await client.writeNote(
      "Notes/Café.md", content: "hello!", baseVersion: .match(note.version))
    #expect(updated.version != note.version)
    let forced = try await client.writeNote(
      "Notes/Café.md", content: "forced", baseVersion: .unconditional)
    ...
    #expect(
      tree.entries == [
        ...
        VaultEntry(
          path: "Notes/Café.md", kind: .file, size: 6, mtime: 1_790_155_800_000,
  ...
  @Test func invalidPathsAndMissingNotes() async throws {
    ...
    for path in [
      "../escape.md", ".obsidian/app.json", ".daily-do-list/x.md", "", "image.png", "a\u{1}b.md",
    ] {
      do {
    ...
    await #expect(
      throws: DaemonClientError.http(
        status: 404, body: ApiErrorBody(error: .notFound, message: "No note at \"missing.md\""))
    ) {
  ...
  @Test func softDeleteMovesToTrashWithCollisionSuffixes() async throws {
    ...
    #expect(
      try await client.deleteNote("Ideas.md")
        == TrashResponse(trashedTo: ".trash/Ideas (2026-09-23 093000).md"))
    _ = try await client.writeNote("Ideas.md", content: "three", baseVersion: .unconditional)
    #expect(
      try await client.deleteNote("Ideas.md")
        == TrashResponse(trashedTo: ".trash/Ideas (2026-09-23 093000 2).md"))
    ...
    await #expect(
      throws: DaemonClientError.http(
        status: 404, body: ApiErrorBody(error: .notFound, message: "No note at \"Ideas.md\""))
    ) {
      try await client.deleteNote("Ideas.md")
    }
  }
  ...
  @Test func renamesNotesAndFolders() async throws {
    let client = Self.client(
      .files(["Projects/a.md": "A", "Projects/sub/b.md": "B", "Other.md": "O", "Taken.md": "T"]))
    let renamed = try await client.rename(from: "Other.md", to: "Archive/Other 2026.md")
    guard case .note(let note) = renamed else {
      throw TimeoutError(description: "expected a note rename")
    }
    ...
    await #expect(
      throws: DaemonClientError.http(
        status: 404, body: ApiErrorBody(error: .notFound, message: "No note at \"nope.md\""))
    ) {
    ...
    await #expect(
      throws: DaemonClientError.http(
        status: 400,
        body: ApiErrorBody(error: .invalidRequest, message: "Source and target are the same"))
    ) {
      try await client.rename(from: "Taken.md", to: "./Taken.md")
    }

    #expect(
      try await client.rename(from: "Projects", to: "Work/Projects")
        == .folder(FolderRenameResponse(path: "Work/Projects", moved: 2)))
    ...
    await #expect(
      throws: DaemonClientError.http(
        status: 409, body: ApiErrorBody(error: .conflict, message: "\"Existing\" already exists"))
    ) {
    ...
    await #expect(
      throws: DaemonClientError.http(
        ...
        body: ApiErrorBody(
          error: .conflict,
          message: "\"Work/Projects/Inner\" already exists or is inside \"Work/Projects\""))
    ) {
  ...
  @Test func foldersAreCreatedAndSoftDeleted() async throws {
    ...
    #expect(
      try await client.createFolder("Empty/Nested") == CreateFolderResponse(path: "Empty/Nested"))
    #expect(
      try await client.createFolder("Empty/Nested") == CreateFolderResponse(path: "Empty/Nested"),
      "idempotent")
    #expect(
      try await client.deleteFolder("Projects") == TrashResponse(trashedTo: ".trash/Projects"))
    let paths = try await client.tree().entries.map(\.path)
    #expect(paths == ["Empty", "Empty/Nested"])
    _ = try await client.writeNote("Projects/a.md", content: "again", baseVersion: .unconditional)
    #expect(
      try await client.deleteFolder("Projects")
        == TrashResponse(trashedTo: ".trash/Projects (2026-09-23 093000)"))
    #expect(try await client.deleteFolder("Empty") == TrashResponse(trashedTo: ".trash/Empty"))
    await #expect(
      throws: DaemonClientError.http(
        status: 404, body: ApiErrorBody(error: .notFound, message: "No folder at \"Nope\""))
    ) {
  ...
  @Test func dailyNotesAreCreatedFromTheTemplateOnce() async throws {
    ...
    await #expect(
      throws: DaemonClientError.http(
        status: 404, body: ApiErrorBody(error: .notFound, message: "No daily note for 2026-09-23"))
    ) {
      try await client.dailyNote("today", create: false)
    }
    let created = try await client.dailyNote("today", create: true)
    #expect(
      created.path == "Daily/2026-09-23.md" && created.date == "2026-09-23" && created.created)
    ...
    await #expect(
      throws: DaemonClientError.http(
        status: 400,
        body: ApiErrorBody(error: .invalidRequest, message: "Date must be \"today\" or YYYY-MM-DD"))
    ) {
  ...
  @Test func todayFollowsTheLocalTimeZone() async throws {
    ...
    let tokyo = InMemoryDaemonClient(
      seed: .empty, clock: .immediate(start: instant, timeZone: TimeZone(identifier: "Asia/Tokyo")!)
    )
    let losAngeles = InMemoryDaemonClient(
      seed: .empty,
      clock: .immediate(start: instant, timeZone: TimeZone(identifier: "America/Los_Angeles")!))
  ...
  @Test func searchMatchesEveryTermNamesFirst() async throws {
    ...
    #expect(
      hits == [
        ...
        SearchHit(
          path: "Groceries.md", kind: .content, line: 1, preview: "- [ ] buy eggs and MILK"),
      ])
    #expect(try await client.search("milk", limit: 2).hits.count == 2)
    #expect(try await client.search("   ", limit: nil).hits.isEmpty)
    #expect(
      try await client.search("notes/oth", limit: nil).hits.first
        == SearchHit(path: "Notes/Other.md", kind: .name, line: 0, preview: "Notes/Other.md"))
    await #expect(throws: DaemonClientError.self) { try await client.search("x", limit: 0) }
    await #expect(throws: DaemonClientError.self) {
      try await client.search(String(repeating: "x", count: 501), limit: nil)
    }
  }
  ...
  @Test func settingsPatchesAreValidatedAndMerged() async throws {
    ...
    let updated = try await client.updateSettings(
      SettingsPatch(
        theme: .dark, editor: .init(fontSize: 13.5),
        agent: .init(model: "  mock/other  ", watch: .init(futureDays: 14))))
    #expect(
      updated.theme == .dark && updated.editor.fontSize == 13.5
        && updated.agent.watch.futureDays == 14)
    #expect(updated.agent.model == "mock/other", "model ids are trimmed")
    #expect(
      updated.agent.watch.pastDays == 0 && updated.editor.vimMode == false,
      "untouched fields keep their values")
    #expect(try await client.settings() == updated)

    for patch in [
      SettingsPatch(editor: .init(fontSize: 400)), SettingsPatch(agent: .init(settleMs: -1)),
      SettingsPatch(agent: .init(maxConcurrentSubagents: 0)),
      SettingsPatch(agent: .init(approvalTimeoutMs: 1000)),
      SettingsPatch(agent: .init(model: "   ")),
      SettingsPatch(agent: .init(watch: .init(pastDays: 400))),
      SettingsPatch(agent: .init(harness: .cursor, cursorModel: " ")),
      SettingsPatch(
        agent: .init(cursorModel: String(repeating: "c", count: SettingsRanges.modelIdLength + 1))),
      SettingsPatch(dailyNotes: .init(folder: ".hidden")),
      SettingsPatch(dailyNotes: .init(folder: "../out")),
    ...
    let settingsEvents = recorder.events.filter {
      if case .settingsChanged = $0 { true } else { false }
    }
  ...
  @Test func theHarnessSwitchesAndTheStatusReportsItsModel() async throws {
    let client = Self.client()
    await client.connect()
    let cursor = try await client.updateSettings(
      SettingsPatch(agent: .init(harness: .cursor, cursorModel: "  gpt-5.5  ")))
    #expect(
      cursor.agent.harness == .cursor && cursor.agent.cursorModel == "gpt-5.5",
    ...
    #expect(
      try await client.updateSettings(SettingsPatch(agent: .init(cursorModel: longest))).agent
        .cursorModel == longest)
  ...
  @Test func ownWritesAreAttributedAndExternalOnesAreNot() async throws {
    ...
    let second = try await client.writeNote(
      "a.md", content: "2", baseVersion: .match(first.version))
    ...
    await client.disconnect()
    try await recorder.waitForFinish()
    let changes = recorder.events.compactMap {
      if case .vaultChanged(let event) = $0 { event } else { nil }
    }
    #expect(
      changes == [
        VaultChangedEvent(
          changes: [VaultChange(path: "a.md", kind: .created, version: first.version)],
          origin: .client, clientId: "macos_test"),
        VaultChangedEvent(
          changes: [VaultChange(path: "a.md", kind: .modified, version: second.version)],
          origin: .client, clientId: "macos_test"),
        VaultChangedEvent(
          changes: [
            VaultChange(path: "a.md", kind: .deleted),
            VaultChange(path: "b.md", kind: .created, version: second.version),
          ],
          origin: .client, clientId: "macos_test"),
        VaultChangedEvent(
          changes: [VaultChange(path: "b.md", kind: .deleted)], origin: .client,
          clientId: "macos_test"),
        VaultChangedEvent(
          changes: [
            VaultChange(
              path: "c.md", kind: .created, version: ContentHash.version(of: "from Obsidian"))
          ], origin: .external),
      ])
  }
  ...
  @Test func eventsFlowOnlyWhileConnected() async throws {
    ...
    #expect(
      before.items.prefix(4) == [
        .state(.idle), .state(.connecting),
        .state(.connected(serverVersion: FakeDaemon.serverVersion)),
  ...
  @Test func demoSeedHasHistoryAndTodaysNote() async throws {
    ...
    let paths = try await client.tree().entries.map(\.path)
    for expected in [
      "Daily", "Projects", "Templates", "Daily/2026-09-23.md", "Daily/2026-09-22.md",
      "Daily/2026-09-21.md",
      "Daily/2026-09-19.md", "Templates/Daily.md", "Ideas.md", "Welcome.md",
    ...
    #expect(
      agentLines.contains {
        $0.hasPrefix("- [ ] Call Trattoria Sole to confirm the table %%agent:thr_")
      })
    #expect(
      records.contains { $0.text == "Call Trattoria Sole to confirm the table" } == false,
      "the agent's task has no record yet")
    let anchored = try #require(records.first { $0.anchor == .line })
    #expect(
      anchored.taskId.hasPrefix("anc_") && anchored.text == "How tall is Ridge Tower downtown?")
    ...
    let answer = question.messages.compactMap {
      if case .text(let text) = $0 { text.text } else { nil }
    }.joined()
    #expect(
      answer.contains("[1](https://city.example/landmarks/ridge-tower)")
        && answer.contains("[[Ideas]]"))
    for record in records where record.anchor == nil {
      ...
      let cited = try #require(
        agentLines.first { $0.hasSuffix("%%agent:\(thread.id)%%") && $0.hasPrefix("  - ") })
      #expect(
        thread.sources?.contains { cited.contains($0.url) } == true,
        "\(cited) cites a source of \(thread.id)")
    }
    ...
    #expect(
      health
        == HealthResponse(
          version: FakeDaemon.serverVersion, apiVersion: 1, vaultName: "Demo Vault",
          agentMode: .mock))
