...
struct FakeHelpersTests {
  ...
  @Test func momentFormatsMatchCoreDates() {
    ...
    #expect(
      calendar.format(LocalDate(year: 2026, month: 9, day: 23), "YYYY/MM/YYYY-MM-DD")
        == "2026/09/2026-09-23")
    #expect(calendar.format(LocalDate(year: 2024, month: 2, day: 29), "DDD ddd E") == "60 Thu 4")
    #expect(
      calendar.format(Date(timeIntervalSince1970: 1_790_155_800), "YYYY-MM-DD HHmmss")
        == "2026-09-23 093000")
  }
  ...
  @Test func dailyNotePathsAndTemplates() throws {
    ...
    #expect(
      try calendar.dailyNotePath(
        date, DailyNoteSettings(folder: "", format: "YYYY/MM/YYYY-MM-DD", template: ""))
        == "2026/09/2026-09-23.md")
    #expect(
      try calendar.dailyNotePath(
        date, DailyNoteSettings(folder: "/Journal//Daily/", format: "", template: ""))
        == "Journal/Daily/2026-09-23.md")
    #expect(throws: FakeVaultPaths.EscapeError.self) {
      try calendar.dailyNotePath(
        date, DailyNoteSettings(folder: "../outside", format: "YYYY", template: ""))
    }
    ...
    let rendered = calendar.renderTemplate(
      "# {{title}}\n{{date:dddd}} {{ TIME }} {{date}} {{unknown}} {{time:HH}}", title: "2026-09-23",
      date: date, now: now)
  ...
  @Test func localDateCalendarMath() {
    ...
    #expect(
      LocalDate(year: 2026, month: 12, day: 31).adding(days: 1)
        == LocalDate(year: 2027, month: 1, day: 1))
    #expect(
      LocalDate(year: 2024, month: 3, day: 1).adding(days: -1)
        == LocalDate(year: 2024, month: 2, day: 29))
  ...
  @Test func trashCandidatesMirrorTheDaemon() {
    #expect(
      FakeVault.trashCandidate("Ideas.md", isFolder: false, stamp: "2026-09-23 093000", attempt: 0)
        == ".trash/Ideas.md")
    #expect(
      FakeVault.trashCandidate(
        "A/Ideas.md", isFolder: false, stamp: "2026-09-23 093000", attempt: 1)
        == ".trash/A/Ideas (2026-09-23 093000).md")
    #expect(
      FakeVault.trashCandidate(
        "A/Ideas.md", isFolder: false, stamp: "2026-09-23 093000", attempt: 2)
        == ".trash/A/Ideas (2026-09-23 093000 2).md")
    #expect(
      FakeVault.trashCandidate("Projects.v2", isFolder: true, stamp: "S", attempt: 1)
        == ".trash/Projects.v2 (S)")
    let long = String(repeating: "é", count: 200) + ".md"
    let candidate = FakeVault.trashCandidate(
      long, isFolder: false, stamp: "2026-09-23 093000", attempt: 1)
