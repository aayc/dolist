...
extension DomainTests {
  ...
  struct TextMergeTests {
    ...
    @Test func describesInsertionsDeletionsAndReplacementsAsHunksOverTheOldLines() {
      ...
      #expect(TextMerge.diffLines(["a", "c"], ["a", "b", "c"]) == [LineHunk(start: 1, end: 1, lines: ["b"])])
      #expect(TextMerge.diffLines(["a", "b", "c"], ["a", "c"]) == [LineHunk(start: 1, end: 2, lines: [])])
      #expect(TextMerge.diffLines(["a", "b", "c"], ["a", "B", "c"]) == [LineHunk(start: 1, end: 2, lines: ["B"])])
    ...
    @Test func linesCompareByCodeUnits() {
      ...
      #expect(TextMerge.diffLines(["caf\u{E9}"], ["cafe\u{301}"]) == [LineHunk(start: 0, end: 1, lines: ["cafe\u{301}"])])
      let merged = TextMerge.merge(base: "caf\u{E9}\nx", local: "caf\u{E9}\ny", remote: "cafe\u{301}\nx")
    ...
    @Test func keepsTheAgentsLinesAndTheUsersTypingWhenTheyTouchDifferentLines() {
      ...
      #expect(
        TextMerge.merge(base: base, local: local, remote: remote)
          == MergeResult(
            text: note(
              "# Thursday", "- [ ] Book a table for two", "  - Trattoria Sole has a table at 7pm %%agent:thr_1%%",
              "- [ ] Renew passport", "Notes"),
            conflict: false))
    }
    ...
    @Test func reportsAConflictWhenBothSidesRewriteTheSameLineKeepingTheUsers() {
      ...
      #expect(TextMerge.merge(base: base, local: local, remote: remote) == MergeResult(text: local, conflict: true))
    }
    ...
    @Test func appliesADeletionNextToAnEdit() {
      ...
      #expect(
        TextMerge.merge(base: base, local: local, remote: remote)
          == MergeResult(text: note("# Thursday", "- [ ] Renew passport", "Notes!"), conflict: false))
    }
    ...
    @Test func anUnchangedSideTakesTheOtherSidesText() {
      ...
      for _ in 0..<300 {
        let base = randomLines(from: Self.lineChoices, maxCount: 12, using: &generator).joined(separator: "\n")
        let other = randomLines(from: Self.lineChoices, maxCount: 12, using: &generator).joined(separator: "\n")
        #expect(TextMerge.merge(base: base, local: base, remote: other) == MergeResult(text: other, conflict: false))
        #expect(TextMerge.merge(base: base, local: other, remote: base) == MergeResult(text: other, conflict: false))
        #expect(TextMerge.merge(base: base, local: other, remote: other) == MergeResult(text: other, conflict: false))
      }
    }
    ...
    @Test func editsToSeparateHalvesOfANoteMergeIntoBothEdits() {
      ...
      var checked = 0
      while checked < 300 {
        let lines = randomLines(from: Self.lineChoices, minCount: 2, maxCount: 12, using: &generator)
          .enumerated().map { "\($1) #\($0)" }
        ...
        let expected = Array(lines[..<i]) + localNew + Array(lines[(i + 1)..<j]) + remoteNew + Array(lines[(j + 1)...])
    ...
    @Test func aDeletionAmongIdenticalLinesCanAlignWithTheOtherSidesEdit() {
      let base = note("  - note", "- [ ] b", "- [ ] b", "- [ ] b", "- [ ] b", "", "## h", "- [ ] b", "- [ ] a")
      let local = note("  - note", "- [ ] b", "- [ ] b", "- [ ] b", "", "## h", "- [ ] b", "- [ ] a")
      let remote = note(
        "  - note", "- [ ] b", "- [ ] b", "- [ ] b", "- [ ] a", "text", "- [ ] a", "", "## h", "- [ ] b", "- [ ] a")
      #expect(TextMerge.merge(base: base, local: local, remote: remote) == MergeResult(text: local, conflict: true))
    }
    ...
    @Test func largeRewritesStayLinearPastTheEditDistanceCap() {
      ...
      #expect(TextMerge.merge(base: base, local: local, remote: remote).text == (["top"] + a + ["bottom"]).joined(separator: "\n"))
    }

    private func randomLines(
      from choices: [String], minCount: Int = 0, maxCount: Int, using generator: inout SeededGenerator
    ) -> [String] {
