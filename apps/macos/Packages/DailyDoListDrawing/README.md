...
| --- | --- | --- |
| `DailyDoListDrawingModel` | The scene model, its JSON codec, the file format, LZ-String, embeds and file names, the text description. Foundation only. | macOS, iOS |
...
- **File format** (`File/`): `ExcalidrawMarkdown` follows `@ddl/core`'s `file.ts`, the reference
  ...
  11.52.33.excalidraw.md`); `DrawingDescription` ports `describe.ts`.
...
Excalidraw's export padding; `DrawingDescription.describe(_:title:)` gives the agent's text.
...
- **Shared fixtures**: `SharedFixtureTests` replays every fixture of
  `packages/core/test/drawings/fixtures` (parse summary, description, byte-exact round trip,
