...
struct BadgeTests {
  ...
  private func remap(
    ...
    index.applyEdit(
      location: range.location, oldLength: range.length,
      newLength: (replacement as NSString).length, text: storage)
    store.applyEdit(
      location: range.location, oldLength: range.length,
      newLength: (replacement as NSString).length, lineIndex: index, text: storage)
    return Dictionary(
  ...
  @Test func linesInsertedOrDeletedAboveShiftBadges() {
    ...
    #expect(
      remap(doc, badges: badges, edit: NSRange(location: 0, length: 0), with: "new\nlines\n") == [
        "a": 3, "b": 4,
      ])
    #expect(
      remap(doc, badges: badges, edit: NSRange(location: 0, length: 6), with: "") == [
        "a": 0, "b": 1,
      ])
    #expect(
      remap(doc, badges: badges, edit: NSRange(location: 36, length: 0), with: "\nmore") == [
        "a": 1, "b": 2,
      ])
  }
  ...
  @Test func editsWithinTheLineKeepTheBadge() {
    ...
    #expect(
      remap(doc, badges: badges, edit: NSRange(location: 12, length: 3), with: "ONE MORE") == [
        "a": 1
      ])
    #expect(
      remap(doc, badges: badges, edit: NSRange(location: 6, length: 0), with: "x") == ["a": 1])
  }

  @Test func enterAtTheStartMovesTheBadgeWithItsTextAndAtTheEndKeepsIt() {
    let badges = [badge("a", line: 1)]
    #expect(
      remap(doc, badges: badges, edit: NSRange(location: 6, length: 0), with: "\n") == ["a": 2])
    #expect(
      remap(doc, badges: badges, edit: NSRange(location: 20, length: 0), with: "\n- [ ] ") == [
        "a": 1
      ])
  }
  ...
  @Test func deletingTheLineDropsTheBadge() {
    ...
    #expect(
      remap(doc, badges: badges, edit: NSRange(location: 6, length: 15), with: "") == ["b": 1])
    // Delete from the end of the previous line to the end of the line.
    #expect(
      remap(doc, badges: badges, edit: NSRange(location: 5, length: 15), with: "") == ["b": 1])
    // Select the content and retype it.
    #expect(
      remap(doc, badges: badges, edit: NSRange(location: 6, length: 14), with: "x") == ["b": 2])
  }
  ...
  @Test func joiningLinesKeepsTheBadgeOnTheMergedLine() {
    ...
    #expect(
      remap(doc, badges: badges, edit: NSRange(location: 20, length: 1), with: "") == ["a": 1])
  }
  ...
  @Test func aReplacementReinsertingTheSameLineKeepsTheBadge() {
    ...
    let swapped = "intro\n- [ ] task two\n- [ ] task one\nend"
    let result = remap(
      doc, badges: badges, edit: NSRange(location: 0, length: (doc as NSString).length),
      with: swapped)
  ...
  @Test func idleAndIgnoredBadgesAreKeptButNotDrawn() {
    ...
    editor.controller.setBadges([
      badge("a", line: 1, status: "idle"), badge("b", line: 2, status: "ignored"),
      badge("c", line: 3, status: "done"),
    ])
    editor.layout()
  ...
  @Test func badgesAreDrawnAfterTheTextWithoutOverlappingIt() throws {
    ...
    editor.controller.setBadges([
      badge("s", line: 0), badge("l", line: 1, status: "waiting_approval"),
    ])
    editor.layout()
    ...
    for layout in layouts {
      ...
      let lineEnd = editor.controller.highlighter.lineIndex.contentRange(
        ofLine: editor.controller.highlighter.lineIndex.line(containing: anchor),
        textLength: (editor.text as NSString).length
      ).end
      let used = layoutManager.lineFragmentUsedRect(
        forGlyphAt: layoutManager.glyphIndexForCharacter(at: lineEnd - 1), effectiveRange: nil)
      #expect(
        layout.rect.minX >= used.maxX + origin.x, "badge \(layout.badge.id) overlaps its line")
      #expect(
        layout.rect.maxX <= editor.textView.bounds.width, "badge \(layout.badge.id) is clipped")
  ...
  @Test func clickingABadgeReportsItWithItsCurrentLine() throws {
    ...
    #expect(
      editor.controller.textView(editor.textView, toolTipAt: point)?.contains("Label a") == true)
  }

  @Test func labelsAreTruncatedAndUnreadCountsCapped() {
    let badge = EditorBadge(
      id: "x", line: 0, status: "done", label: String(repeating: "abc ", count: 20), unread: 150)
  ...
  @Test func pillsShrinkToTheSpaceTheyHave() {
    ...
    let badge = EditorBadge(
      id: "x", line: 0, status: "working", label: "Book a table for four", unread: 2)
  ...
  @Test func unreadMessagesAddADotNotACount() {
    ...
    many.unread = 150
    #expect(
      renderer.width(of: one) == renderer.width(of: many), "the dot doesn't grow with the count")
  ...
  @Test func onlyBadgesThatNeedYouAreLoud() {
    ...
    for status in ["triaging", "queued", "working"] {
      #expect(BadgeTier(status: status) == .working)
    }
    for status in ["done", "cancelled", "a_future_status"] {
      #expect(BadgeTier(status: status) == .quiet)
    }
  ...
  @Test func badgesInANarrowEditorAreShortenedInsteadOfClipped() throws {
    ...
    for layout in layouts {
      #expect(
        layout.rect.maxX <= editor.textView.bounds.width, "badge \(layout.badge.id) is clipped")
      #expect(
        layout.label.isEmpty || layout.label == layout.badge.displayLabel
          || layout.label.hasSuffix("…"))
    }
    #expect(
      layouts.contains { $0.label != $0.badge.displayLabel },
      "200pt is too narrow for the full labels")
    let first = try #require(layouts.first)
    let point = NSPoint(x: first.rect.midX, y: first.rect.midY)
    #expect(
      editor.controller.textView(editor.textView, toolTipAt: point)?.contains(label) == true,
      "the tooltip keeps the full label")
  }

  @Test func narrowEditorsReserveARightMarginForBadges() {
    let editor = EditorHarness(
      text: doc, configuration: EditorConfiguration(readableLineLength: false),
