...
struct HitTestingTests {
  @Test func checkboxRectsSitOnTheTaskLines() throws {
    ...
    for box in boxes {
      ...
      let lineRect = editor.controller.lineRectInTextView(at: editor.controller.highlighter.lineIndex.start(ofLine: box.line))
  ...
  @Test func clickingACheckboxTogglesTheTaskAsAnUndoableEdit() throws {
    ...
    editor.act { #expect(editor.controller.handleClick(at: NSPoint(x: first.rect.midX, y: first.rect.midY), modifiers: [])) }
    ...
    editor.act { editor.controller.handleClick(at: NSPoint(x: second.rect.midX, y: second.rect.midY), modifiers: []) }
    #expect(editor.text == "- [x] one\n- [x] two\nplain")
    editor.undo()
    #expect(editor.text == "- [x] one\n- [/] two\nplain")
  ...
  @Test func readOnlyCheckboxesSwallowClicksWithoutToggling() throws {
    ...
    #expect(editor.controller.handleClick(at: NSPoint(x: box.rect.midX, y: box.rect.midY), modifiers: []))
  ...
  @Test func plainClickFollowsRenderedLinks() {
    let text = "See [[Daily/2026-06-19#Tasks|today]] and [docs](https://example.com/docs) and [note](Notes/Plan.md)\nx"
    let editor = EditorHarness(text: text, selection: NSRange(location: (text as NSString).length, length: 0))
    editor.layout()
    #expect(editor.controller.handleClick(at: editor.point(at: editor.range(of: "today")), modifiers: []))
    #expect(editor.delegate.wikiLinks.map(\.target) == ["Daily/2026-06-19"])
    #expect(editor.delegate.wikiLinks.first?.newWindow == false)
    #expect(editor.controller.handleClick(at: editor.point(at: editor.range(of: "docs")), modifiers: []))
    #expect(editor.delegate.links == [URL(string: "https://example.com/docs")!])
    #expect(editor.controller.handleClick(at: editor.point(at: editor.range(of: "note")), modifiers: []))
    #expect(editor.delegate.wikiLinks.last?.target == "Notes/Plan.md")
    #expect(!editor.controller.handleClick(at: editor.point(at: editor.range(of: "See")), modifiers: []))
  }
  ...
  @Test func linksOnTheCaretLineNeedCommandClick() {
    ...
    #expect(editor.controller.handleClick(at: editor.point(at: editor.range(of: "example.com")), modifiers: .command))
  ...
  @Test func unsafeSchemesAreNeverPassedOn() {
    ...
    #expect(LinkClassifier.classify("me@example.com") == .external(URL(string: "mailto:me@example.com")!))
    #expect(LinkClassifier.classify("tel:+123") == .external(URL(string: "tel:+123")!))
    #expect(LinkClassifier.classify("Notes/My%20Plan.md#Goals") == .note(target: "Notes/My Plan.md", subpath: "Goals"))
    #expect(LinkClassifier.destination(for: .wiki(target: "", subpath: "H", alias: nil, isEmbed: false)) == nil)
  }
}
