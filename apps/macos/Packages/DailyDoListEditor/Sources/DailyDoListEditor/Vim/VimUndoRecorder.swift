...
final class VimUndoStep {
  ...
  init(
    inverse: VimChangeSet, startSelection: VimSelection, below: VimUndoStep?, actionName: String?
  ) {
...
final class VimUndoRecorder {
  ...
  func record(
    _ changes: VimChangeSet, inverse: VimChangeSet, startSelection: VimSelection,
    userEvent: String?, time: Double,
    ...
    if let top, !top.inverse.isEmpty, Self.isJoinable(userEvent),
      (top.selectionsAfter.isEmpty && time - prevTime < Self.newGroupDelay
        && Self.isAdjacent(top.inverse, inverse))
        || userEvent == "input.type.compose"
    {
      ...
      if manager.canRedo {
        ...
        let step = VimUndoStep(
          inverse: joined, startSelection: top.startSelection, below: top.below,
      ...
      let step = VimUndoStep(
        inverse: inverse, startSelection: startSelection, below: top,
        actionName: Self.actionName(userEvent))
      controller.registerVimUndoStep(step, in: manager)
      top = step
    }
    prevTime = time
  ...
  func recordSelection(_ startSelection: VimSelection, userEvent: String?, time: Double) {
    ...
    if let previous = last.last, time - prevTime < Self.newGroupDelay, userEvent == prevUserEvent,
      ...
      Self.sameShape(previous, startSelection)
    {
      return
    }
  ...
  static func isAdjacent(_ earlier: VimChangeSet, _ later: VimChangeSet) -> Bool {
    ...
    return later.changedRanges.contains { range in
      ranges.contains { range.toB >= $0.0 && range.fromB <= $0.1 }
    }
  }
