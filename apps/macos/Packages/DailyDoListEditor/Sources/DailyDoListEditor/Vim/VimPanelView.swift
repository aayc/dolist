...
final class VimPanelView: NSView {
  ...
  func show(_ panel: VimPanel) {
    kind = panel.kind
    let attributes: [NSAttributedString.Key: Any] = [.font: Self.font, .foregroundColor: EditorColors.panelText]
    let muted: [NSAttributedString.Key: Any] = [.font: Self.font, .foregroundColor: EditorColors.panelMutedText]
    let result = NSMutableAttributedString()
    switch panel.kind {
    case .prompt:
      result.append(NSAttributedString(string: panel.text + panel.value.string, attributes: attributes))
    ...
    case .message:
      caretX = nil
      result.append(NSAttributedString(string: panel.text, attributes: [.font: Self.font, .foregroundColor: EditorColors.danger]))
      if panel.isLong {
  ...
  func fittingHeight(forWidth width: CGFloat) -> CGFloat {
    ...
    let bounds = text.boundingRect(with: NSSize(width: available, height: .greatestFiniteMagnitude), options: [.usesLineFragmentOrigin])
  ...
  override func draw(_ dirtyRect: NSRect) {
    ...
    let area = NSRect(
      x: Self.padding.width, y: Self.padding.height + 1, width: max(0, bounds.width - 2 * Self.padding.width),
      height: max(0, bounds.height - 2 * Self.padding.height - 1))
    // A long message shows its end when it doesn't fit.
    let needed = text.boundingRect(with: NSSize(width: area.width, height: .greatestFiniteMagnitude), options: [.usesLineFragmentOrigin])
    ...
    text.draw(with: NSRect(origin: origin, size: NSSize(width: area.width, height: needed.height)), options: [.usesLineFragmentOrigin])
    ...
    if let caretX {
      ...
      NSRect(x: (area.minX + min(caretX, area.width)).rounded(), y: top, width: 2, height: lineHeight.rounded()).fill()
    }
  }
}
