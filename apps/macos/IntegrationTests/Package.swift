...
// drive it through HTTPDaemonClient. Some launch a second daemon (the always-on machine) and the
// sync service (apps/sync/dist/main.js). Run with `apps/macos/scripts/test.sh integration`;
// skipped (with a message) when Node.js 24.4+ or the built daemon (or sync service) is missing.
let package = Package(
