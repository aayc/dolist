...
test.describe("typing markdown with a real keyboard", () => {
  test("today's note opens with the caret after the template, ready for a task", async ({
    ...
    expect(await daemon.read(dailyPath())).toBe("- [ ] Buy oat milk");
  });

  test("types task checkboxes, links and continues lists on Enter", async ({ page, daemon }) => {
    await openApp(page);
    ...
    expect(await daemon.read(dailyPath())).toBe(
      [
        "- [ ] Book dentist",
