...
- **Daemon startup:** the daemon bundle is code-split; heavy optional dependencies load on first
 ...
 statically elsewhere: `apps/daemon/build.mjs` fails the build if Pi's packages
 (`@earendil-works/*`) or `playwright-core` would load before the daemon answers. It doesn't check
 the Cursor harness, so keep `@ddl/agent/cursor` behind `import()` yourself. The one static agent
 import is `@ddl/agent/routines` (routine files, editable while no agent runs here): keep that
 entry free of harness, execution and model code.
