...
| S4 web Settings, the orchestrator toggle, pairing screen | `feat/always-on-web` | fullstack e2e 12 passed against S1+S2 (`97aa769`); `feat/always-on` (relay, journal) merged in at `770d95a`; now enabling the relay test and aligning the mock with the relay's messages (browser pairing over https stays fixme: the harness has no TLS proxy) |
