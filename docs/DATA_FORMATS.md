...
| --- | --- | --- | --- | --- |
| `threads/<threadId>.json` | `packages/agent/src/threads/journal/migrate.ts` | JSON, compact (written by older apps; moved into the journal) | 1 | yes |
...
4. **Quarantine corrupt.** An empty file, invalid JSON (truncated, garbage), a top level that is
   ...
   `state/records.json` → `corrupt/state/records.20260923T215800123Z.json` (`-2`, `-3`, … on
...
This app no longer writes the file (threads are
[journal-only](#thread-journal--statejournalthreadsthreadidjsonl)); older apps did, and the shape
lives on as the thread a journal's `thread.imported` event carries. When the thread store loads, it
moves every snapshot into the thread's journal (`packages/agent/src/threads/journal/migrate.ts`):

- Only direct children of `threads/` ending in `.json` are read. A file named like an id
  (`thr_a.json`) must contain that thread. Other names, such as a sync conflict copy
  `thr_a (conflict 2026-09-23 1830).json`, count for the thread they contain.
- A thread's snapshots are imported the way older apps merged them on load: fold the journal, then
  for each snapshot in merge order (its own file, then copies from the most recently updated), a
  `thread.imported` event if the snapshot holds something the fold lacks (messages the journal
  trimmed don't count). Merging (`mergePersistedThreads`) never loses anything: messages and
  artifacts are unioned by id (ours wins for an id present on both sides; entries only on the other
  side are interleaved by `createdAt`), surfaces are unioned, and title/status/taskId/notePath
  come from the copy updated last. It is idempotent.
- The event's id is a hash of the snapshot's content, so two devices migrating the same file write
  the same line (kept once by the union merge), and an import already in the journal counts as
  held. Once the journal, read back, holds every planned event, the snapshot is deleted,
  conditionally: one rewritten meanwhile (an older app on another device) waits for the next load.
  A partly invalid snapshot is copied into `corrupt/` first (rule 5).
- Unreadable snapshots and ones a newer app wrote are left untouched and reported, every load.
  Nothing is written to a journal a newer app wrote, and no journal is started for a thread whose
  own snapshot a newer app wrote.
...
The only persisted form of a thread: an append-only log of events, one compact JSON object per
line, `\n`-terminated. The thread is the fold of its events (`foldJournal` in
`packages/agent/src/threads/journal/fold.ts`, which the thread store, the read-only view and the
import all use). It lives under the agent-owned `state/` folder, so the agent lease's fencing
covers it, and outside `threads/`, where older apps wrote snapshots. Schema:
...
- **Thread events** (the fold): `thread.created` (`thread`: id, taskId, notePath, title, status,
  ...
  messages the journal trimmed stay trimmed; a migrated snapshot's has an id derived from its
  content), `message` (added, or replacing the one with its id), `status`, `title`, `trim`
  (`keep` the newest messages), `surface`, `sources` (merged by URL, newest kept, capped at 50),
  `artifact` (metadata; the body is a file as before). `message`, `status` and `artifact` move
  `updatedAt` forward (never back).
...
- **Reading.** A line that isn't JSON, isn't an object, has an invalid `v`, an unknown `type` or
  ...
  without a thread event is folded from what follows (a placeholder header).
- **Writing.** Events are appended in debounced batches (400 ms; streaming deltas never schedule
  one), with `StorageProvider.append` (conditional on the version last seen; local-fs versions
  journals by stat, so an append never re-reads the file). Streaming text is journaled once final (a flush
...
- **Loading.** The store first moves snapshots older apps wrote into their journals
  ([Thread](#thread--threadsthreadidjson)) and merges conflict copies of journals a third-party
  sync made (`thr_a 2.jsonl`: the events the journal lacks are appended as they are, then the copy
  goes), then folds every `<threadId>.jsonl`. Another device's events are folded in at the next
  append; journals aren't watched live.
...
the real owner in `packages/agent/test/persistence/*.test.ts` (thread store and the snapshot
migration, task records, approval broker, task watcher) and `apps/daemon/src/settings-store.test.ts`.
...
- Threads are not reloaded live when another device changes them; its events fold in at the next
  append or on restart. (Settings are reloaded live, see Settings.)
- Sync conflict copies of single-file state (`records (conflict …).json`, `approvals`, `settings`)
  are left for the user; only thread snapshots' and journals' copies are merged.
