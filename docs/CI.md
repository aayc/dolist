...
| `Integration tests and Swift format` | strict swift-format (`node scripts/lint.mjs --all --only swift`), builds the daemon and the sync service, `test.sh integration` (the real daemon with the mock agent, the demo's daemon, two synced daemons through the real sync service) |
