...
| macOS app (`macos.yml`) | push to `main` and pull requests touching the app, the daemon, the sync service, what they bundle or the vim vectors; manual (inputs `release`, `thorough`) | `packages` (`app`, `editor`, `core`), `integration`, `ios`, `release` (main or `release`) |
...
| `Swift packages (editor)` | `test.sh DailyDoListUI DailyDoListVim DailyDoListDrawing DailyDoListEditor`, the vim vector replays included |
| `Swift packages (core)` | `test.sh DailyDoListModels DailyDoListDomain DailyDoListClient DailyDoListAgent DailyDoListDaemon DailyDoListComputer`, then a smoke test of the debug `ddl-computer` |
